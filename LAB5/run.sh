...
echo "Log_filter,"$header  > log-MPI.csv
echo "Flip_filter,"$header > flip-MPI.csv
...
for i in {1..40}
do
    mpirun -np $i ./bin/log     -c MPI -i $INPUT_IMAGE -n $i >> log-MPI.csv
    mpirun -np $i ./bin/flip -H -c MPI -i $INPUT_IMAGE -n $i >> flip-MPI.csv
done
